...
interface InviteMemberComboboxProps {
  users: any;
...
const InviteMemberCombobox: React.FC<InviteMemberComboboxProps> = ({
  users,
  onSelect,
          ...
          <Command>
            ...
            <CommandList>
              {Array.isArray(users) && users.length > 0 ? (
                <CommandGroup>
                  {users.map((user: any) => (
                    <CommandItem key={user.id} value={user.member.first_name} onSelect={() => handleUserSelect(user)}>
                      <Check
                        className={cn(
                          'mr-2 h-4 w-4',
                          selectedUser === user.member.first_name ? 'opacity-100' : 'opacity-0',
                        )}
                      />
                      <Text size={'xs'}>
...
export default InviteMemberCombobox;
